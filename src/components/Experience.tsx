...
const Card = ({ item, index, progress }: { item: typeof EXP_DATA[0]; index: number; progress: MotionValue<number> }) => {
  ...
  return (
    <motion.div
      className="absolute inset-0 w-full h-full flex flex-col justify-start pt-[18vh] pb-[4vh] lg:justify-end lg:pt-0 lg:pb-[8vh] xl:pb-[10vh] px-6 md:px-12 xl:px-24"
      style={{
    ...
    >
      <div className="w-full h-full max-w-[1500px] mx-auto flex flex-col lg:grid lg:grid-cols-[1fr_auto_1fr] gap-4 lg:gap-8 items-start lg:items-end">
        
        {/* Lado Esquerdo: Index & Title */}
        <div className="flex items-start lg:items-center gap-3 lg:gap-6 lg:pb-12 shrink-0">
          <span className={`text-xs lg:text-sm font-bold pt-2 lg:pt-0 ${item.accentText}`}>{item.id}</span>
          <h3 className="text-4xl md:text-5xl lg:text-5xl xl:text-6xl text-white font-medium tracking-tight font-sans">
            {item.company}
          </h3>
        </div>

        {/* Centro: Imagem */}
        <div className="flex-1 flex justify-center items-center w-full py-4 lg:py-0 lg:pb-[4vh]">
          <div className="relative h-[25vh] md:h-[30vh] lg:h-[35vh] xl:h-[42vh] max-h-[380px] aspect-square shadow-2xl overflow-hidden bg-white/5 border border-white/5 rounded-sm mx-auto">
            <Image 
              src={item.image} 
              alt={item.company} 
              fill 
        ...
        <div className="flex flex-col gap-6 lg:gap-14 lg:pb-12 lg:pl-8 xl:pl-20 shrink-0 mt-auto lg:mt-0 w-full lg:w-auto">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-3">
            {item.tags.map((tag, i) => (
              <div key={i} className="flex items-start gap-2 group cursor-default">
                <span className={`${item.accentText} font-bold text-[11px] lg:text-xs`}>/</span>
                <span className="text-white/60 font-bold text-[11px] lg:text-xs tracking-[0.08em] uppercase leading-relaxed font-sans group-hover:text-white transition-colors duration-300">
                  {tag}
                </span>
              </div>
            ))}
  ...
  return (
    <section id="experiencia" className="relative w-full h-[400vh]" ref={containerRef}>
      ...
      <div className="sticky top-0 w-full h-[100vh] overflow-hidden bg-[#181528]">
        ...
        <div className="absolute top-[6vh] md:top-[8vh] lg:top-[8vh] left-0 w-full text-center z-50 pointer-events-none opacity-90">
          <h2 className="font-bebas uppercase text-[12vh] md:text-[14vh] lg:text-[16vh] xl:text-[18vh] leading-none text-white tracking-tight transition-colors duration-200" style={{ textShadow: "1px 1px 0px #000, 2px 2px 0px #000, 3px 3px 0px #000, 4px 4px 0px #00D2FF, 5px 5px 0px #00D2FF, 6px 6px 0px #00D2FF" }}>
